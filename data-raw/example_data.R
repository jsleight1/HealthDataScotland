...
example_gp_data <- c(
  ...
  filter(.data[["PracticeCode"]] %in% c("10002", "10017"))
...
example_hospital_data <- get_phs_dataset("d719af13-5fb3-430f-810e-ab3360961107") |>
  ...
  filter(.data[["Location"]] %in% c("A101H", "A201H"))

example_hospital_metadata <- get_phs_dataset("c698f450-eeed-41a0-88f7-c1e40a568acc") |>
  filter(.data[["HospitalCode"]] %in% c("A101H", "A201H"))

example_gp_sf <- sf::read_sf("inst/extdata/scotland_gps.json")
